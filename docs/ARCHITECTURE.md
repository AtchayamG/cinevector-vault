...
## 1. System Overview
**CineVector Vault** is a video intelligence and vector continuity engine powered by **Google Gemini 2.5 Flash** and the official **ClickHouse MCP Server (`mcp-clickhouse`)**. It evaluates character facial, wardrobe, and lighting drift across AI-generated video shots.

```mermaid
graph TD
    User([🎬 User / Script Supervisor]) --> UI[🖥️ Interactive Web UI<br/>Shot & Scene Selector]
    UI --> GeminiAgent[🤖 Gemini 2.5 Flash Visual Feature Extractor<br/>(Extracts costume, lighting & facial tokens)]
...
2. **Gemini 2.5 Flash Extraction**: Calls `google-genai` to deconstruct the shot into structured attributes (costume items, lighting palette, keypoints).
