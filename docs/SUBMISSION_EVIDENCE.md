...
| **Active Live Gemini/Vertex Execution Proof** | Authenticated Live Gemini Run | **BLOCKED** | Provide credentials in `.env` | Blocked until a credential-backed call is recorded with live execution. |
...
| **Live Cluster Proof** | Live authenticated connection to ClickHouse cluster | **PASS** | Authenticated live smoke call | Official mcp-clickhouse runtime execution against self-hosted ClickHouse is PASS. ClickHouse Cloud-specific proof is optional and not performed. |
